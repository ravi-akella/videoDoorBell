...
class AudioData:
    ...
    def audio_init(self):
        for file in glob.glob("/home/pi/videoDoorBell/refImages/*.mp3"):
            name = file.split("/")[-1]
            name = name.split(".")[0]
            self.audio_file_dict[name] =  file
            print(self.audio_file_dict[name])
        self.mixer = pygame.mixer
        self.mixer.init()
        
    def play_audio(self,name):
        print(list(self.audio_file_dict.keys()))
        if name in list(self.audio_file_dict.keys()):
            self.mixer.music.load(self.audio_file_dict[name])
            self.mixer.music.play()
            print("audio played")
            
    def is_audio_trigger_needed(self,name):
        if (name == self.prev_face):
            self.prev_face_count+=1
            return
        elif name != self.prev_face:
    ...
    if face_data is not None:
        ...
        for ((x,y,w,h),name) in zip(faces_roi,names_to_display):
            ...
            if not name == "Unknown":
                ...
                audio.is_audio_trigger_needed(name)
